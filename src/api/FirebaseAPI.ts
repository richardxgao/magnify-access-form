...
export const checkIfEmployeeExists = async (employee: Employee): Promise<boolean> => {
  const employeeEmailQuery = await getDocs(query(employeesRef, where("email", "==", employee.email)));
  const employeeIdQuery = await getDocs(query(employeesRef, where("id", "==", employee.id)));

  return employeeEmailQuery.size > 0 || employeeIdQuery.size > 0;
};
